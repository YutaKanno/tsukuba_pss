"""スタッツ作成モード - 投手ごとのスタッツ一覧テーブル（全体 / 対右 / 対左）"""
import io
import os
import datetime
import matplotlib
...
def _page_header(team_name, start_date, end_date, font) -> Table:
    title = _para('スタッツ一覧', font, size=15, color=colors.white,
...
# ── スタッツ計算 ────────────────────────────────────────────────
def _build_stats_df(df_team: pd.DataFrame, batter_side) -> pd.DataFrame:
    """チームの全投手スタッツを投球回降順でまとめた DataFrame を返す。"""
    pitchers = df_team['投手氏名'].dropna().unique().tolist()
    rows = []
    for pitcher in pitchers:
        df_p = df_team[df_team['投手氏名'] == pitcher].copy()
        # 投球データ（打撃結果 != '0'）が存在しない投手はスキップ
        if df_p[df_p['打撃結果'] != '0'].empty:
            continue
        rows.append(calc_overallStats(df_p, batter_side, pitcher))
    if not rows:
        return pd.DataFrame()
    df_stats = pd.DataFrame(rows)
    df_stats = df_stats.sort_values('投球回', ascending=False).reset_index(drop=True)
    return df_stats
...
def _generate_stats_pdf(
    ...
    for i, (batter_side, label) in enumerate([
        (None, '全体'),
        ('右', '対右打者'),
        ('左', '対左打者'),
    ]):
        ...
        df_stats = _build_stats_df(df_team, batter_side)
        if df_stats.empty:
            story.append(_para(f'{label} のデータがありません。', font))
            continue

        fig = plot_overallStatsTable(df_stats)
...
def show():
    ...
    with col_start:
        start_date = st.date_input('開始日', value=date_min, key='sm_start')
    with col_end:
        end_date = st.date_input('終了日', value=date_max, key='sm_end')
    ...
    with col_team:
        selected_team = st.selectbox('守備チーム', teams, index=team_idx, key='sm_team')
    ...
    with tab_export:
        st.markdown('**PDF**')
        if st.button('PDF 生成', key='sm_gen_pdf'):
            ...
            st.download_button(
                ...
                file_name=f'stats_{selected_team}_{start_date}_{end_date}.pdf',
