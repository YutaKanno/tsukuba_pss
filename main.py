...
if st.session_state.page_ctg == "start":
    ...
    if st.session_state.get( "pending_analysis_select" ):
        ...
        with _ac3:
            if st.button( "⚾ 投手分析", type = "primary", use_container_width = True ):
                ...
                st.rerun()
        with _ac4:
            if st.button( "🏃 打者分析", type = "primary", use_container_width = True ):
                st.session_state.page_ctg = "batter_analysis_mode"
                st.session_state.pop( "pending_analysis_select", None )
                st.rerun()
        with _ac5:
    ...
    with st.expander( "📖 使い方ガイド（クリックで展開）", expanded = False ):
        g_tab1, g_tab2, g_tab3, g_tab4, g_tab5 = st.tabs( [ "🔰 基本操作", "📋 入力データ一覧", "⚙️ メニュー機能", "💾 データ出力", "📊 データ分析" ] )
...
スタートページの **「データ分析」** ボタンから起動します（試合データが1件以上ある場合に有効）。
...
| モード | 状態 | 内容 |
|--------|------|------|
| ⚾ 投手分析 | 利用可 | 自チーム・対戦相手の投手を詳細分析 |
| 🏃 打者分析 | 準備中 | — |
| 📋 スコア表 | 利用可 | 試合ごとのスコアブック・投手スコアを確認 |
...
**フィルター**
- **期間**（開始日・終了日）で集計範囲を絞り込めます
- **守備チーム**・**投手氏名** を選択して対象投手を切り替えます

**タブ構成**

| タブ | 内容 |
|------|------|
| 全体 | 総合スタッツ・球速分布・被打球性質・登板履歴 |
| vs 右打者 | 右打者限定のスタッツ・球種割合・コース分布 |
| vs 左打者 | 左打者限定のスタッツ・球種割合・コース分布 |
| コメント | 投手ごとのメモ（チーム内で共有・保存） |
| 出力 | PDF・PowerPoint（PPTX）形式でレポートを出力 |

**各タブの詳細**

- **全体タブ**
  - 総合スタッツ（投球回・奪三振・四死球・被打率・OPS 等）を全体／vs右／vs左／平均値で比較
  - 球速分布（球種別ヴァイオリン図）
  - 被打球性質（ゴロ率・フライ率等の棒グラフ、全投手平均と比較）
  - 登板履歴（試合日時・対戦相手・投球回・投球数・打者数・被安打・四死球・失点を一覧表示）

- **vs 右打者 / vs 左打者タブ**
  - スタッツ表・全体球種割合パイチャート
  - カウント別球種割合（B-S グリッド形式）
  - コース分布（球種別：コース分布・詳細・打球方向）

- **コメントタブ**
  - 投手ごとにメモを記入・保存できます
  - 「Edit」ボタンで編集モードに切り替え、「保存」で確定

- **出力タブ**
  - **PDF 生成** → ページをまたぐ長い登板履歴も自動レイアウト
  - **PPTX 生成** → PowerPoint ファイルとしてダウンロード
  - 両形式とも：総合スタッツ・球速・被打球・登板履歴・コメント（1ページ目）＋ vs右打者・vs左打者（以降のページ）
...
**操作手順**
1. 「チーム」ドロップダウンでチームを選択します
2. 「試合」ドロップダウンで対象試合を選択します

**表示内容**

| セクション | 内容 |
|-----------|------|
| スコア | 1〜9回（延長あれば以降も）のイニング別得点 ＋ H/E/K/B 集計 |
| 先攻スコアブック | 打順・守備位置・選手名・打席左右・各回の打席結果 |
| 後攻スコアブック | 同上（後攻チーム） |
| 投手スコア | 先攻・後攻それぞれの投手ごとに 投球数・打者数・H・K・B |

**スコアブックの見方**
- 代打・選手交代が発生した場合、元の選手の直下に交代選手の行が追加されます
- 同一打順で守備位置が変わった場合は「P → LF」のように矢印で表示します
- 打者一巡して同じ打順が同じ回に再び打席に立った場合は「単打 / 三振」のように / 区切りで表示します

**PDF 出力**
- 「PDF 生成」ボタンを押すとスコア・スコアブック・投手スコアを1ページにまとめた PDF が生成されます
- 「PDF ダウンロード」ボタンでファイルをダウンロードできます

> **注意**：自チームが入力した側の打席データのみが記録されている場合、相手チームの投手スコアが「データがありません」と表示されることがあります。
""" )
